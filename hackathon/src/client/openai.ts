...
export class OpenAI {
    ...
    chatCompletion = async (systemMessage: string, prompt: string, model: GPTModel): Promise<string> => {
        ...
        console.info(`Creating completion for ${tokens.length} tokens`)
    ...
    imageChatCompletion = async (base64: string, prompt: string): Promise<string> => {
        try {
            ...
            // TODO: Fix typing
            const { choices } = await this.openai.chat.completions.create(opts as any)
    ...
    createCompletion = async (prompt: string, temperature: number): Promise<string> => {
        const tokens: number[] = encode(prompt)
        console.info(`Creating completion for ${tokens.length} tokens`, { prompt: `${prompt.substring(0, 20)}...` })
    ...
    createEmbedding = async (input: string | string[]): Promise<Array<OpenAIAPI.Embeddings.Embedding>> => {
        ...
        if (isString) {
            ...
            console.info(`Creating embedding for string with ${tokens} tokens`)

            if (tokens >= 8185) throw new Error(`Input is too long for $${input.slice(0, 40)}...}`)
        } else {
            console.info(`Creating embedding for ${input.length} inputs`)
        }
        ...
        console.info('Created embedding.')
